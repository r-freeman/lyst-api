...
<header class="flex items-center justify-between flex-wrap mx-8 my-3">
    <div class="flex items-center flex-shrink-0">
        <h1 class="text-4xl font-black text-mineshaft">Edit List</h1>
    </div>
</header>
{{-- header end --}}
...
<div class="flex justify-center">
    <div class="w-full max-w-sm my-24">
        ...
        <form method="POST" action="{{ route('user.lists.update', $list->id) }}" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            <input type="hidden" name="_method" value="PUT">
            <input type="hidden" name="_token" value="{{ csrf_token() }}">
            <div class="my-4">
                <input id="name" type="text" name="name" value="{{ old('name', $list->name) }}"
